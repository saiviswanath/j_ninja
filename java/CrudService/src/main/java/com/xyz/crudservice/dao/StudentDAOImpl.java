...
public class StudentDAOImpl implements StudentDAO {
  ...
  public List<String> findCoursesByStudentId(int studentId) {
    ...
    String sql =
        "select c.coursename from Student s inner join "
  ...
  public List<StudentDto> findAllStudents() {
    ...
    String sql =
        "select studentId, firstName, lastName, gender, DOB, email, "
  ...
  public List<Integer> findCourseIdsByCourseNames(List<String> courseNames) {
    ...
    String sql = "select CourseId from Course where CourseName in (" + sb.toString() + ")";
    try (Connection connection = DBConnector.getDBConnection()) {
      PreparedStatement stmt = connection.prepareStatement(sql);
      int i = 0;
  ...
  public void createStudent(StudentDto student) {
    ...
    try {
      ...
      String studentInsertSql =
          "insert into Student(FirstName, LastName, Gender, DOB, Email, "
              + "mobilenumber, Address) values(?, ?, ?, ?, ?, ?, ?)";
      PreparedStatement studentInsertStmt = connection.prepareStatement(studentInsertSql);
      ...
      PreparedStatement retrieveStudentIdStmt = connection.prepareStatement(retrieveStudentIdSql);
      ...
      insertIntoStuCourseMappingWithStudIdAndCourses(connection, studentId, student.getCourses());
  ...
  public StudentDto findByName(String firstName, String lastName) {
    String sql =
        "select studentId, gender, DOB, email, mobileNumber, address from Student "
  ...
  private void insertIntoStuCourseMappingWithStudIdAndCourses(Connection connection, int studentId,
      List<String> courses) throws SQLException {
    List<Integer> courseIds = findCourseIdsByCourseNames(courses);
    String insertStuCourseMappingSql =
        "insert into student_course_mapping(StudentId, " + "CourseId) values(?, ?)";
    PreparedStatement insertStuCourseMappingStmt =
        connection.prepareStatement(insertStuCourseMappingSql);
  ...
  private void deleteFromStuCourseMappingWithStudIdAndCourses(Connection connection, int studentId,
      List<String> courses) throws SQLException {
    List<Integer> courseIds = findCourseIdsByCourseNames(courses);
    String deleteStuCourseMappingSql =
        "delete from student_course_mapping where studentId=? and courseId=?";
    PreparedStatement deleteStuCourseMappingStmt =
        connection.prepareStatement(deleteStuCourseMappingSql);
    for (Integer courseId : courseIds) {
  ...
  public void updateStudent(StudentDto student) {
    ...
    String sql =
        "update Student set firstname=?, lastname=?, gender=?, DOB=?, Email=?, mobilenumber=?, "
    ...
    try {
      ...
      int studentId = findStudentIdByName(student.getFirstName(), student.getLastName());
      ...
      if (coursesToAdd.size() > 0) {
        insertIntoStuCourseMappingWithStudIdAndCourses(connection, studentId, coursesToAdd);
      }

      if (coursesToDelete.size() > 0) {
        deleteFromStuCourseMappingWithStudIdAndCourses(connection, studentId, coursesToDelete);
      }
  ...
  public boolean deteteStudentByName(String firstName, String lastName) {
    ...
    try {
      ...
      deleteFromStuCourseMappingWithStudIdAndCourses(connection, studentId, courses);
